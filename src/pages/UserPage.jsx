...
export const UserPage = (props) =>{
        ...
        const {data,error,loading,getData} = useFetch(`${process.env.REACT_APP_BACKEND_URL}/users?name=${search}`,token,false)
        

        const triggerSearch = async () =>{
          

            await getData(`${process.env.REACT_APP_BACKEND_URL}/users?name=${search}`,token)
            
           
        }

       useEffect(() => {
         getData(`${process.env.REACT_APP_BACKEND_URL}/users?name=${search}`,token)
